    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col relative">
      <header className="bg-slate-900 border-b border-slate-800 px-6 py-4 flex flex-col xl:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-wrap bg-slate-950 p-1 rounded-xl border border-slate-800 gap-1">
            <button onClick={() => setActiveTab('pos')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${activeTab === 'pos' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}>🛒 Caja POS</button>
            <button onClick={() => setActiveTab('inventory')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition flex items-center gap-1.5 ${activeTab === 'inventory' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}>
              📦 Inventario
              {lowStockCount > 0 && <span className="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.2 rounded-full">{lowStockCount}</span>}
            </button>
            <button onClick={() => setActiveTab('accounts')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${activeTab === 'accounts' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}>📒 Cuentas (Cobrar/Pagar)</button>
            <button onClick={() => setActiveTab('reports')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${activeTab === 'reports' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}>📊 Reportes Z</button>
            <button onClick={() => setActiveTab('roles')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition flex items-center gap-1.5 ${activeTab === 'roles' ? 'bg-cyan-600 text-white' : 'text-cyan-400 hover:text-white bg-cyan-950/40 border border-cyan-800/50'}`}>🛡️ Roles y Personal</button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 px-3 py-1.5 rounded-xl text-xs">
            ...
            <input type="number" step="0.01" value={exchangeRate} onChange={(e) => setExchangeRate(parseFloat(e.target.value) || 0)} className="bg-slate-900 text-white font-bold w-20 px-2 py-0.5 rounded border border-slate-700 text-center" />
          </div>
          ...
          <div className="flex items-center gap-2.5 bg-slate-950 border border-slate-800 px-3 py-1.5 rounded-xl text-xs">
            ...
            <div className="ml-2 pl-2 border-l border-slate-800 flex items-center gap-1.5">
              <select value={currentUsername} onChange={(e) => setCurrentUsername(e.target.value)} className="bg-slate-900 text-xs text-white border border-slate-700 rounded px-2 py-1 font-medium cursor-pointer">
                {usersList.map(u => {
        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 max-w-7xl mx-auto w-full">
          <div className="lg:col-span-7 flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <input type="text" placeholder="Buscar producto por nombre..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white" />
              <div className="flex gap-1 overflow-x-auto pb-1 sm:pb-0">
                {categories.map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition ${selectedCategory === cat ? 'bg-blue-600 text-white' : 'bg-slate-900 border border-slate-800 text-slate-400'}`}>{cat}</button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-[550px] overflow-y-auto pr-1">
              {filteredProducts.map(product => {
                ...
                return (
                  <button key={product.id} onClick={() => addToCart(product)} className={`bg-slate-900 border p-4 rounded-2xl text-left transition flex flex-col justify-between group shadow-lg ${isOut ? 'border-red-500/30 opacity-60 cursor-not-allowed' : 'border-slate-800 hover:border-blue-500/60'}`}>
                    <div>
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] uppercase font-semibold text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded">{product.category}</span>
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${product.taxable ? 'text-amber-400 bg-amber-500/10' : 'text-emerald-400 bg-emerald-500/10'}`}>{product.taxable ? 'IVA 16%' : 'Exento'}</span>
                      </div>
                      <div className="font-semibold text-slate-200 mt-2 text-sm">{product.name}</div>
                    </div>
                    <div className="mt-4 flex justify-between items-end">
                      ...
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded ${isOut ? 'bg-red-500/20 text-red-400' : 'bg-slate-800 text-slate-300'}`}>Stk: {product.stock}</span>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="lg:col-span-5 bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between shadow-xl">
            <div>
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                {cart.length === 0 && <div className="text-center py-8 text-slate-500 text-sm">No hay productos en el ticket.</div>}
                {cart.map(item => {
                    ...
                    <div key={item.id} className="bg-slate-950/60 border border-slate-800/60 p-3 rounded-xl flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex items-center border border-slate-800 rounded-lg bg-slate-900">
                          <button onClick={() => updateQuantity(item.id, -1)} className="px-2 py-1 text-slate-400 text-xs">-</button>
            ...
            <div className="border-t border-slate-800 pt-4 mt-4 space-y-3">
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-1.5 text-sm">
                <div className="flex justify-between text-slate-400"><span>Subtotal:</span><span>${Number(subtotalUSD || 0).toFixed(2)}</span></div>
                <div className="flex justify-between text-slate-400"><span>IVA (16%):</span><span>${Number(totalIvaUSD || 0).toFixed(2)}</span></div>
              ...
              <button onClick={() => setIsCheckoutModalOpen(true)} disabled={cart.length === 0} className={`w-full py-3.5 rounded-xl font-bold transition shadow-lg ${cart.length > 0 ? 'bg-blue-600 hover:bg-blue-500 text-white cursor-pointer' : 'bg-slate-800 text-slate-500 cursor-not-allowed'}`}>
                Procesar Venta 💳
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-5">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setIsCheckoutModalOpen(false)} className="text-slate-400 bg-slate-800/60 p-2 rounded-xl text-xs">✕</button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as PaymentMethodType)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white font-semibold text-blue-300">
              ...
              {paymentMethod === 'Crédito / Fiado' ? (
                <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-2.5">
                  ...
                  <div>
                    ...
                    <input type="text" value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Ej. Juan Pérez" className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white" />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] text-slate-400 mb-0.5">Cédula / RIF</label>
                      <input type="text" value={clientDocument} onChange={(e) => setClientDocument(e.target.value)} placeholder="V-12345678" className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white" />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-400 mb-0.5">Teléfono</label>
                      <input type="text" value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} placeholder="0414-0000000" className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white" />
                    </div>
                  </div>
                </div>
              ) : (
                <div>
                  ...
                  <div className="flex justify-between items-center bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5">
                    <input type="number" step="0.1" value={cashGivenUSD} onChange={(e) => setCashGivenUSD(e.target.value)} placeholder="0.00" className="bg-transparent text-white focus:outline-none w-full text-sm" />
                    <div className="text-right">
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setIsCheckoutModalOpen(false)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-3 rounded-xl text-xs transition">Cancelar</button>
              <button onClick={handleCheckout} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl text-xs transition shadow-lg">Completar Cobro ⚡</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-sm w-full p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              ...
              <button onClick={() => { setIsRestockModalOpen(false); setSelectedProductForRestock(null); }} className="text-slate-400 bg-slate-800/60 p-2 rounded-xl text-xs">✕</button>
            </div>
            ...
            <form onSubmit={handleRestockSubmit} className="space-y-3">
              <div>
                ...
                <input type="number" min="1" required value={restockAmount} onChange={(e) => setRestockAmount(e.target.value)} placeholder="Ej. 24" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white font-bold" />
              </div>
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => { setIsRestockModalOpen(false); setSelectedProductForRestock(null); }} className="flex-1 bg-slate-800 text-slate-300 font-bold py-2.5 rounded-xl text-xs">Cancelar</button>
                <button type="submit" className="flex-1 bg-emerald-600 text-white font-bold py-2.5 rounded-xl text-xs shadow-lg">Sumar Stock 📦</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex bg-slate-900 border border-slate-800 p-1 rounded-xl gap-1">
                <button onClick={() => setInventoryFilterMode('all')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${inventoryFilterMode === 'all' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}>Todos ({products.length})</button>
                <button onClick={() => setInventoryFilterMode('low')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 ${inventoryFilterMode === 'low' ? 'bg-red-600 text-white' : 'text-slate-400'}`}>⚠️ Stock Bajo ({lowStockCount})</button>
              </div>
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
            ...
            <form onSubmit={handleAddProduct} className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              <div className="sm:col-span-2">
                ...
                <input type="text" required value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Ej. Maltín Polar" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white" />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Costo ($)</label>
                <input type="number" step="0.01" required value={newCostPrice} onChange={(e) => setNewCostPrice(e.target.value)} placeholder="0.00" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white" />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Venta ($)</label>
                <input type="number" step="0.01" required value={newPrice} onChange={(e) => setNewPrice(e.target.value)} placeholder="0.00" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white" />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Stock</label>
                <input type="number" required value={newStock} onChange={(e) => setNewStock(e.target.value)} placeholder="0" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white" />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Categoría</label>
                <select value={newCategory} onChange={(e) => setNewCategory(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white">
              ...
              <div className="flex items-center gap-2 pt-5">
                <input type="checkbox" id="tax" checked={newTaxable} onChange={(e) => setNewTaxable(e.target.checked)} className="w-4 h-4 rounded bg-slate-950 border-slate-800 text-blue-600" />
              ...
              <div className="sm:col-span-2 lg:col-span-5 flex items-end">
                <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 rounded-xl text-sm transition">Guardar Producto</button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-800 text-sm">
                {inventoryProducts.length === 0 && <tr><td colSpan={8} className="text-center py-12 text-slate-500">No hay productos que mostrar en este filtro.</td></tr>}
                  ...
                  return (
                    <tr key={prod.id} className="hover:bg-slate-800/40 transition">
                      <td className="p-4 font-medium text-white flex items-center gap-2">
                        {prod.name}
                        {isLow && <span className="bg-red-500/20 text-red-400 border border-red-500/30 text-[10px] font-bold px-2 py-0.5 rounded-full animate-pulse">⚠️ Stock Bajo</span>}
                      </td>
                      ...
                      <td className="p-4 text-right space-x-2">
                        <button onClick={() => { setSelectedProductForRestock(prod); setIsRestockModalOpen(true); }} className="bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 px-3 py-1.5 rounded-lg text-xs font-semibold">+ Stock</button>
                        <button onClick={() => deleteProduct(prod.id)} className="bg-red-500/10 text-red-400 hover:bg-red-500/20 px-3 py-1.5 rounded-lg text-xs font-semibold">Eliminar</button>
                      </td>
                    </tr>
                  );
                })}
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xl flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-center border-b border-slate-800 pb-3 mb-4">
                  ...
                  <span className="text-xs bg-amber-500/10 text-amber-400 font-bold px-2 py-1 rounded">Pendientes: ${Number(pendingCreditsUSD || 0).toFixed(2)}</span>
                </div>
                <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1">
                  {credits.length === 0 && <div className="text-center py-12 text-slate-500 text-sm">No hay cuentas por cobrar registradas.</div>}
                  {credits.map(credit => (
                    <div key={credit.id} className="bg-slate-950 border border-slate-800/80 p-3.5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${credit.status === 'Pendiente' ? 'bg-amber-500/10 text-amber-400' : 'bg-emerald-500/10 text-emerald-400'}`}>{credit.status}</span>
                        </div>
                        <div className="text-[11px] text-slate-400">Doc: <strong className="text-slate-300">{credit.clientDocument}</strong> • Tel: <strong className="text-slate-300">{credit.clientPhone}</strong></div>
                      </div>
                      <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                        ...
                        {credit.status === 'Pendiente' && <button onClick={() => payCredit(credit.id)} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-1.5 rounded-lg text-xs transition shadow">Saldar 💰</button>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xl flex flex-col justify-between">
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                  ...
                  <span className="text-xs bg-red-500/10 text-red-400 font-bold px-2 py-1 rounded">Pendientes: ${Number(pendingPayablesUSD || 0).toFixed(2)}</span>
                </div>
                <form onSubmit={handleAddPayable} className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input type="text" required value={newProviderName} onChange={(e) => setNewProviderName(e.target.value)} placeholder="Nombre Proveedor *" className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white" />
                    <input type="text" value={newProviderDoc} onChange={(e) => setNewProviderDoc(e.target.value)} placeholder="RIF / Cédula" className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <input type="text" value={newPayableDesc} onChange={(e) => setNewPayableDesc(e.target.value)} placeholder="Concepto" className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white sm:col-span-1" />
                    <input type="number" step="0.01" required value={newPayableAmountUSD} onChange={(e) => setNewPayableAmountUSD(e.target.value)} placeholder="Monto USD ($) *" className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white" />
                    <input type="text" value={newDueDate} onChange={(e) => setNewDueDate(e.target.value)} placeholder="Fecha Límite" className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white" />
                  </div>
                  <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 rounded-lg text-xs transition">Guardar Cuenta x Pagar 📥</button>
                </form>

                <div className="space-y-3 max-h-[260px] overflow-y-auto pr-1">
                  {payables.length === 0 && <div className="text-center py-6 text-slate-500 text-xs">No hay cuentas por pagar registradas.</div>}
                    ...
                    <div key={payable.id} className="bg-slate-950 border border-slate-800/80 p-3.5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${payable.status === 'Pendiente' ? 'bg-red-500/10 text-red-400' : 'bg-emerald-500/10 text-emerald-400'}`}>{payable.status}</span>
                        </div>
                        <div className="text-[11px] text-slate-400">Concepto: <strong className="text-slate-300">{payable.description}</strong> • Vence: <strong className="text-amber-400">{payable.dueDate}</strong></div>
                      </div>
                      <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                        <div className="text-right">
                        ...
                        {payable.status === 'Pendiente' && <button onClick={() => payPayable(payable.id)} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-1.5 rounded-lg text-xs transition shadow">Pagar ✅</button>}
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {activeTab === 'reports' && (
        <main className="flex-1 p-6 max-w-7xl mx-auto w-full space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h2 className="text-2xl font-bold">Dashboard de Analítica y Cierre Z Pro</h2>
              <span className="text-sm text-slate-400">Auditoría financiera, tendencias de facturación y métodos de pago</span>
            </div>
            {salesHistory.length > 0 && (
              <button onClick={downloadReportZ} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-4 py-2.5 rounded-xl text-xs transition shadow-lg flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-4">
              <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wider">📈 Tendencia de Ventas (Historial Reciente)</h3>
              <div className="h-72 w-full">
                {salesHistory.length === 0 ? (
                  <div className="flex items-center justify-center h-full text-slate-500 text-xs">No hay datos suficientes para graficar.</div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={salesHistory.slice(-10)}>
                      ...
                      <Line type="monotone" dataKey="totalUSD" stroke="#3b82f6" strokeWidth={3} dot={{ fill: '#3b82f6', r: 4 }} name="Total ($)" />
                    </LineChart>
                  </ResponsiveContainer>
                )}
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-4">
              <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wider">💳 Distribución por Método de Pago ($)</h3>
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={[
                    ...
                    <YAxis stroke="#64748b" tick={{ fontSize: 10 }} />
                    <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '8px', color: '#fff', fontSize: '12px' }} />
                    <Bar dataKey="total" fill="#06b6d4" radius={[6, 6, 0, 0]} name="Monto ($)" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl shadow">
              ...
              <div className="text-lg font-black">Bs. {Number(statsEfectivoUSD.totalBs || statsPagoMovil.totalBs || 0).toFixed(2)}</div>
