      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex flex-col xl:flex-row justify-between items-center gap-4 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-xl text-xs">
            ...
            <input type="number" step="0.01" value={exchangeRate} onChange={(e) => setExchangeRate(parseFloat(e.target.value) || 0)} className="bg-white text-slate-900 font-bold w-20 px-2 py-0.5 rounded border border-slate-300 text-center" />
          </div>
          ...
          <div className="flex items-center gap-2.5 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-xl text-xs">
            ...
            <div className="ml-2 pl-2 border-l border-slate-200 flex items-center gap-1.5">
              <select value={currentUsername} onChange={(e) => setCurrentUsername(e.target.value)} className="bg-white text-xs text-slate-900 border border-slate-300 rounded px-2 py-1 font-medium cursor-pointer">
                {usersList.map(u => {
        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 max-w-7xl mx-auto w-full">
          <div className="lg:col-span-7 flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <input type="text" placeholder="Buscar producto por nombre..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 shadow-sm" />
              <div className="flex gap-1 overflow-x-auto pb-1 sm:pb-0">
                {categories.map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition ${selectedCategory === cat ? 'bg-blue-600 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-100'}`}>{cat}</button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-[550px] overflow-y-auto pr-1">
              {filteredProducts.map(product => {
                ...
                return (
                  <button key={product.id} onClick={() => addToCart(product)} className={`bg-white border p-4 rounded-2xl text-left transition flex flex-col justify-between group shadow-sm ${isOut ? 'border-red-200 opacity-60 cursor-not-allowed' : 'border-slate-200 hover:border-blue-400 hover:shadow-md'}`}>
                    <div>
                    ...
                    <div className="mt-4 flex justify-between items-end">
                      ...
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded ${isOut ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-600'}`}>Stk: {product.stock}</span>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="lg:col-span-5 bg-white border border-slate-200 rounded-2xl p-6 flex flex-col justify-between shadow-md">
            <div>
              <h2 className="text-lg font-bold mb-4 border-b border-slate-100 pb-3 flex justify-between items-center text-slate-800">
                    ...
                    <div key={item.id} className="bg-slate-50 border border-slate-200 p-3 rounded-xl flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex items-center border border-slate-300 rounded-lg bg-white">
                          <button onClick={() => updateQuantity(item.id, -1)} className="px-2 py-1 text-slate-600 text-xs hover:bg-slate-100">-</button>
            ...
            <div className="border-t border-slate-100 pt-4 mt-4 space-y-3">
              ...
              <button onClick={() => setIsCheckoutModalOpen(true)} disabled={cart.length === 0} className={`w-full py-3.5 rounded-xl font-bold transition shadow-md ${cart.length > 0 ? 'bg-blue-600 hover:bg-blue-500 text-white cursor-pointer' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
                Procesar Venta 💳
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-5">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              ...
              <button onClick={() => setIsCheckoutModalOpen(false)} className="text-slate-400 hover:text-slate-600 bg-slate-100 p-2 rounded-xl text-xs">✕</button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as PaymentMethodType)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-xs text-slate-900 font-semibold">
              ...
              {paymentMethod === 'Crédito / Fiado' ? (
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-2.5">
                  <div className="text-xs font-bold text-amber-600">Datos del Cliente (Crédito)</div>
                  <div>
                    <label className="block text-[10px] text-slate-500 mb-0.5">Nombre y Apellido *</label>
                    <input type="text" value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Ej. Juan Pérez" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-900" />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-0.5">Cédula / RIF</label>
                      <input type="text" value={clientDocument} onChange={(e) => setClientDocument(e.target.value)} placeholder="V-12345678" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-900" />
                    </div>
                    <div>
                      <label className="block text-[10px] text-slate-500 mb-0.5">Teléfono</label>
                      <input type="text" value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} placeholder="0414-0000000" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-900" />
                    </div>
                  </div>
                </div>
              ) : (
                <div>
                  ...
                  <div className="flex justify-between items-center bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5">
                    ...
                    <div className="text-right">
                      <span className="text-xs text-slate-400">Vuelto: <strong className="text-emerald-600">${Number(changeUSD || 0).toFixed(2)}</strong></span>
      ...
      {successModalData && successModalData.isOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-5 text-center">
            <div className="w-16 h-16 bg-emerald-50 border border-emerald-200 text-emerald-600 rounded-full flex items-center justify-center mx-auto text-2xl font-bold">
              ✓
            </div>
            <div>
              <h3 className="text-xl font-bold text-slate-900">¡Venta Procesada con Éxito!</h3>
            ...
            <div className="space-y-2 pt-2">
              <button onClick={() => { handlePrintReceipt(); setSuccessModalData(null); }} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl text-xs transition shadow-md flex items-center justify-center gap-2">
                🖨️ Imprimir Recibo
              </button>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl max-w-sm w-full p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              ...
              <button onClick={() => { setIsRestockModalOpen(false); setSelectedProductForRestock(null); }} className="text-slate-400 bg-slate-100 p-2 rounded-xl text-xs">✕</button>
            </div>
            ...
            <form onSubmit={handleRestockSubmit} className="space-y-3">
              <div>
                ...
                <input type="number" min="1" required value={restockAmount} onChange={(e) => setRestockAmount(e.target.value)} placeholder="Ej. 24" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-900 font-bold" />
              </div>
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" className="flex-1 bg-emerald-600 text-white font-bold py-2.5 rounded-xl text-xs shadow-md">Sumar Stock 📦</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex bg-white border border-slate-200 p-1 rounded-xl gap-1 shadow-sm">
                <button onClick={() => setInventoryFilterMode('all')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${inventoryFilterMode === 'all' ? 'bg-blue-600 text-white' : 'text-slate-600'}`}>Todos ({products.length})</button>
                <button onClick={() => setInventoryFilterMode('low')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 ${inventoryFilterMode === 'low' ? 'bg-red-600 text-white' : 'text-slate-600'}`}>⚠️ Stock Bajo ({lowStockCount})</button>
              </div>
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-md">
            ...
            <form onSubmit={handleAddProduct} className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              <div className="sm:col-span-2">
                ...
                <input type="text" required value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Ej. Maltín Polar" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-900" />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Costo ($)</label>
                <input type="number" step="0.01" required value={newCostPrice} onChange={(e) => setNewCostPrice(e.target.value)} placeholder="0.00" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-900" />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Venta ($)</label>
                <input type="number" step="0.01" required value={newPrice} onChange={(e) => setNewPrice(e.target.value)} placeholder="0.00" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-900" />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Stock</label>
                <input type="number" required value={newStock} onChange={(e) => setNewStock(e.target.value)} placeholder="0" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-900" />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Categoría</label>
                <select value={newCategory} onChange={(e) => setNewCategory(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-900">
                  ...
                  return (
                    <tr key={prod.id} className="hover:bg-slate-50 transition">
                      <td className="p-4 font-medium text-slate-900 flex items-center gap-2">
                        {prod.name}
                        {isLow && <span className="bg-red-50 text-red-600 border border-red-200 text-[10px] font-bold px-2 py-0.5 rounded-full animate-pulse">⚠️ Stock Bajo</span>}
                      </td>
                      ...
                      <td className="p-4"><span className={`font-bold px-2 py-1 rounded text-xs ${isLow ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-800'}`}>{prod.stock} un.</span></td>
                      <td className="p-4 text-right space-x-2">
                        <button onClick={() => { setSelectedProductForRestock(prod); setIsRestockModalOpen(true); }} className="bg-emerald-50 text-emerald-700 hover:bg-emerald-100 px-3 py-1.5 rounded-lg text-xs font-semibold">+ Stock</button>
                        <button onClick={() => deleteProduct(prod.id)} className="bg-red-50 text-red-600 hover:bg-red-100 px-3 py-1.5 rounded-lg text-xs font-semibold">Eliminar</button>
                      </td>
                    </tr>
                  );
                })}
                    ...
                    <div key={credit.id} className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${credit.status === 'Pendiente' ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>{credit.status}</span>
                        </div>
              ...
              <div className="space-y-4">
                ...
                <form onSubmit={handleAddPayable} className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input type="text" required value={newProviderName} onChange={(e) => setNewProviderName(e.target.value)} placeholder="Nombre Proveedor *" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-900" />
                    <input type="text" value={newProviderDoc} onChange={(e) => setNewProviderDoc(e.target.value)} placeholder="RIF / Cédula" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-900" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <input type="text" value={newPayableDesc} onChange={(e) => setNewPayableDesc(e.target.value)} placeholder="Concepto" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-900 sm:col-span-1" />
                    <input type="number" step="0.01" required value={newPayableAmountUSD} onChange={(e) => setNewPayableAmountUSD(e.target.value)} placeholder="Monto USD ($) *" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-900" />
                    <input type="text" value={newDueDate} onChange={(e) => setNewDueDate(e.target.value)} placeholder="Fecha Límite" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-900" />
                  </div>
                    ...
                    <div key={payable.id} className="bg-slate-50 border border-slate-200 p-3.5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${payable.status === 'Pendiente' ? 'bg-red-50 text-red-700' : 'bg-emerald-50 text-emerald-700'}`}>{payable.status}</span>
                        </div>
